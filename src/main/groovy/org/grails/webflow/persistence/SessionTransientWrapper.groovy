 ...
 */
class SessionTransientWrapper implements Session, SessionImplementor {

    protected transient final SessionImplementor sessionImplementor;
    protected transient final Session session;

    public SessionTransientWrapper(Session session) {
        if ( session == null ) {
            throw new IllegalArgumentException( "Unable to create a SessionTransientWrapper from a null session object" );
        }
        this.sessionImplementor = session instanceof SessionImplementor ? session : null;
